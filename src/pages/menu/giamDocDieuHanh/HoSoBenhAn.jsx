...
import ThongKe from '../../NhomChucNang/HoSoBenhAn/ThongKe';
import DanhSach from '../../NhomChucNang/HoSoBenhAn/DanhSach';
import ThongTinHoSoBenhAn from '../../NhomChucNang/HoSoBenhAn/ThongTinHoSoBenhAn';
            ...
            {tabConfig.map((tab, index) => (
                <button
                    ...
                    onClick={() => {
                        ...
                        navigate(`/ho-so-benh-an/${tab.path}`);
                    }}
...
function App() {
    ...
    useEffect(() => {
        const currentTab = tabConfig.find(tab => `/ho-so-benh-an/${tab.path}` === location.pathname);
        if (currentTab) {
    ...
    return (
        <div className="w-full min-h-screen flex flex-col">
            ...
            <div className="bg-blue-600 text-white p-2 px-3 py-3 w-full rounded text-left font-bold sticky top-0 z-10">
                <h1>QUẢN LÝ THÔNG TIN HỒ SƠ BỆNH ÁN</h1>
            </div>
